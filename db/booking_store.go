...
type BookingStore interface {
	...
	GetRoomBookings(ctx context.Context, filter bson.M) ([]*types.Booking, error)
...
func (s *MongoBookingStore) GetRoomBookings(ctx context.Context, filter bson.M) ([]*types.Booking, error) {
	resp, err := s.coll.Find(ctx, filter)
	if err != nil {
		return nil, err
	}
	var bookedRoom []*types.Booking
	if err := resp.All(ctx, &bookedRoom); err != nil {
		return nil, err
	}
	return bookedRoom, nil
}
